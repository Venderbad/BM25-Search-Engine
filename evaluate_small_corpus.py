...
def load_global_vars():
    ...
    with open(os.path.join(os.path.dirname(__file__), "qrels.txt"), "r") as f:
        ...
        for line in lines:
            ...
            docname = tokens[2]
            grade = tokens[3]
            if int(grade) > 0: # this will always be true in small corpus
...
def cal_bpref(ret_rank_ls: List, rel_rank_ls: List)-> float:
    ret_set = set(ret_rank_ls)
    rel_set = set(rel_rank_ls)
    sum = 0
    non_rel_sofar = set()
...
if __name__ == "__main__":
    ...
    for q_id in all_ret_ls_dict:
        ret_ls = all_ret_ls_dict[q_id]
        rel_ls = all_rel_ls_dict[q_id]
        ...
        results["bprefs"].append(cal_bpref(ret_ls, rel_ls))
