"""
This is the main file of the search engine, which should be run as the main program. Usages are available by running `python search_large_corpus.py -h` or `python search_large_corpus.py --help`.

Author: Venderbad
Version: large-corpus-0.0.4
"""
...
rank_cap:int = 35 # the maximum number of results to show
...
def rebuild_index():
    ...
    for root, dirs, filenames in os.walk(docs_dir):
        for dir in dirs:
            for subroot, subdirs, subfilenames in os.walk(os.path.join(root, dir)):
                for subfilename in subfilenames:
                    update_index_by_file(subroot, subfilename)
